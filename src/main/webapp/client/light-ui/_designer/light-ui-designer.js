...
//					this._designerPane;
					<!--根据结构信息 创建layout-->
//					rootPanel.createChildren(paneKey,{
//						north: {
//							spacing_open:0,
//							size:30
//						},
...
//						},
//						south__onresize:function (pane1, $pane, state1, options) {
//							designerPropertyPanel.tabs( "option", "heightStyle", "fill" );
//						}
//					});
					<!--创建设计器按钮 及树的脚本结构-->
...
//					this._types._filter = {dependencies:[{'type-name':"dataFilters"}]};
//					this._types._sort = {dependencies:[{'type-name':"dataSorts"}]};
					//添加字段选项
					menuHtml+='<li _type_name="_field"><a href="#"><span class="ui-icon"></span>字段</a></li>';
//					//添加过滤条件选项
//					menuHtml+='<li _type_name="_filter"><a href="#"><span class="ui-icon"></span>过滤条件</a></li>';
//					//添加排序条件选项
//					menuHtml+='<li _type_name="_sort"><a href="#"><span class="ui-icon"></span>排序条件</a></li>';
							...
							if(type_name == "_field"){
								...
								if(selectedNode.component.name == 'formFields'){
									...
									if(formNode.component.type == 'form'){
										...
										if(dataset==null){
											LUI.Message.info("错误","未找到关联数据源对象，请保存后刷新页面，再重试！!");
											return;
										}
										...
										LUI.Mapping.openGridMaping(dataset,selectedNode);
									}else{
										//使用数据源字段选择窗口 选择下级字段
										var datasourceNode = LUI.PageDesigner.instance.getDatasourceBySelectedNode(selectedNode);
...
//													LUI.PageDesigner.instance._pageCmpTree.expandNode(newNode.getParentNode());
									...
									if(datasourceNode.data.component == 'todoDataset' || datasourceNode.data.component == 'javaDataset'){
										...
										var gridRenderTemplate = gridNode.data.renderTemplate;
										if(gridRenderTemplate==null){
											LUI.Message.info("错误","当前表格未设置构建模板!");
											return;
										}
							...
							}else if(type_name == "_button"){
								//添加按钮
								if(selectedNode.component.name == 'formButtons'){
									//为form添加按钮
								}
							}else{
				...
				addPropertyToDatasetNode:function(datasetNode,fieldsNode,newFieldArray){
					...
					if(datasetNode.component.name == 'gnDataset' || datasetNode.component.name == 'gnRecord' ){
						for(var i=0;i<LUI.PageDesigner.instance._gongNengs.length;i++ ){
							...
							if(gongNengDefData.gongNengDH == datasetNode.data.gongNengDH){
					...
					}else if(datasetNode.component.name == 'stlDataset' || datasetNode.component.name == 'stlRecord' ){
						shiTiLeiDef = LUI.PageDesigner.instance.getShiTiLeiDefByDH(datasetNode.data.shiTiLeiDH);
...
//					LUI.PageDesigner.instance._pageCmpTree.selectNode(newNodeArray[0],false);
					...
					for(var i=0;i<structureDef.length;i++ ){
						...
						if(structure_component_def.isArray == "true"){
							...
							this.addComponentNode(newNodeArray[0],structureName,nodeLabel+'-数据源');
						}
					}
					return newNodeArray[0];
				},
...
//				},
				onComponentNodeSelected:function(selectedNode){
					if(this._editingNode!=null && this._editingNode == selectedNode){
						return;
					}else if(this._editingNode!=null){
				...
				applyDataToTree:function(treeData){
					...
					if(treeChildData!=null){
						for(var i=0;i<treeChildData.length;i++){
							var component_def = LUI.PageDesigner.instance._components[treeChildData[i].component];
							if(component_def==null){
								LUI.Message.info("信息","component("+treeChildData[i].component+")定义不存在!");
							}
							var type_def = LUI.PageDesigner.instance._types[component_def.type];
							if(component_def==null){
...
//							iconSkin:"designer-tree-changelog",
						...
						this._tabs.tabs({ heightStyle: "fill" });
						<!--隐藏控件信息tab...-->
				...
				getDatasourceBySelectedNode:function(node){
					...
					while(cmpNode.getParentNode()!=null && cmpNode.getParentNode().component!=null && cmpNode.getParentNode().component.type !='page'){
						...
						if( typeName == 'record' || typeName == 'dataset' ){
							...
							|| componentName == 'singleEditForm' || componentName == 'dataDisplayForm' ){
	...
	_designerContent:'<div class="ui-layout-north" style="overflow: hidden;">'+
						...
						'<input id="_designer-tools-property-toggle" type="checkbox" checked><label for="_designer-tools-property-toggle" style="height:24px"></label>'+
	...
	if(!event.params.isInitial){
		...
		if(componentPropertiesArray.length >0){
			node.form.properties_2 = LUI.Form.createNew({
				name:node.id+'_properties_form2',
				renderto:'#_designer_tab_properties_form2',
				fields:componentPropertiesArray
			},true);
			
			for(var i=0;i<componentPropertiesArray.length;i++){
				if(!node.record.hasField(componentPropertiesArray[i].name)){
					node.record.addField(componentPropertiesArray[i]);
				}
			}
		...
		if(componentExtensionsArray.length >0){
			node.form.extensions_2 = LUI.Form.createNew({
				name:node.id+'_extensions_form2',
				renderto:'#_designer_tab_extensions_form2',
				fields:componentExtensionsArray
			},true);
			
			for(var i=0;i<componentExtensionsArray.length;i++){
				if(!node.record.hasField(componentExtensionsArray[i].name)){
					node.record.addField(componentExtensionsArray[i]);
				}
			}
		...
		if(structureDef.length ==0 && node.children!=null && node.children.length > 0){
			//删除下级节点
			for(var i=0;i<node.children.length;i++ ){
				var childNode = node.children[i];
				LUI.PageDesigner.instance._pageCmpTree.removeNode(childNode);
			}
		}else if(structureDef.length > 0 && (node.children==null || node.children.length == 0)){
			//添加下级节点
			for(var i=0;i<structureDef.length;i++ ){
				var structureName = structureDef[i]['component-name'];
				if(structureName==null){
				...
				if(structure_component_def.isArray == "true"){
					...
					//这里假设组件下的结构节点非数组的情况只有一种：私有数据源
					LUI.PageDesigner.instance.addComponentNode(node,structureName,nodeLabel+'-数据源');
				}
			}
		}
...
//根据当前选中节点 取得关联数据源的属性列表 供选择
function getDatasetPropertyOptions(){
	var options = [];
	//当前选中的节点
	var selectedNodes = LUI.PageDesigner.instance._pageCmpTree.getSelectedNodes();
	var selectedNode = selectedNodes[0];
 ...
 * @returns {Array}
 */
function setDisplayGridRenderTo(eventSource,eventTarget,event,eventOriginal){
	var newVal = event.params.newValue;
	var oldVal = event.params.oldValue;
	var isInitial = event.params.isInitial;
	...
	if(newVal!=null && newVal != oldVal && !isInitial){
		...
		var selectedNode = selectedNodes[0];
		//标题行
		var headerLineValue = selectedNode.record.getFieldValue('headerLines');
		//目标元素
		var targetUL = $("#_pageContent "+newVal+" li:eq("+headerLineValue+")");
		//以第一个li为准 是需要迭代的
		var fieldValue = selectedNode.record.getFieldValue('renderTemplate');
		if(fieldValue == null || fieldValue.length==0){
			var rowContent = $("<p>").append(targetUL.clone()).html();
			selectedNode.record.setFieldValue('renderTemplate',rowContent);
		}
		//根据li内有id 的 span 为当前表格创建column 作为下级节点 添加在当前树形结构下
//		targetUL.find("span,div").each(function(index,el){
//			if(el.id !=null && el.id.length >0){
//				//查找selectedNode的子节点 是否有名称一致的
//				for(var i=0;i<selectedNode.children.length;i++){
//					var node = selectedNode.children[i];
//					//为同名节点设置renderto（原值为空的情况下）
//					if(node.data.name == el.id && (node.data.renderto == null || node.data.renderto.length ==0)){
//						node.data.renderto = el.id;
//						break;
//					}
//				}
//			}
...
function changeFieldSourceType(eventSource,eventTarget,event,eventOriginal){
	...
	var hidden = eventSource.form.record.getFieldValue('hidden');
	if(hidden != 'true'){
		if(newVal == 'public'){
			//显示公共数据源字段 
